...
function checkAchievementCondition() {
  database.ref('progress').once('value', snapshot => {
    const data = snapshot.val();
    let totalProgress = 0;

    if (data) {
      Object.values(data).forEach(entry => {
        totalProgress += entry.history.length;
      });
    }

    // Misal achievement ketika total progress 5 entri
    if (totalProgress >= 5) {
      database.ref('achievement/unlocked').set(true);
      console.log("Achievement unlocked!");
    } else {
      database.ref('achievement/unlocked').set(false);
    }
  });
}
...
function deleteProgress(key) {
  database.ref('progress').child(key).remove()
    .then(() => {
      ...
      checkAchievementCondition(); // Recheck after deletion
    })
    .catch(err => console.error('Delete error:', err));
}

// ========== Start ==========
document.addEventListener('DOMContentLoaded', fetchRecommendations);// ========== Firebase Setup ==========
const firebaseConfig = {
  apiKey: "AIzaSyAog5ZF6Ybbsm3LrtbNxumLfY5u2DPjhOg",
  authDomain: "hive-b6642.firebaseapp.com",
  databaseURL: "https://hive-b6642-default-rtdb.firebaseio.com",
